...
async function installApi(page: Page, options: { readonly empty?: boolean; readonly notifications?: boolean } = {}) {
  ...
  await page.route("**/api/v2/**", async (route: Route) => {
    ...
    if (matchedWorkspace && path === `/api/v2/workspaces/${matchedWorkspace.id}` && method === "PATCH") {
    ...
    if (matchedWorkspace && path === `/api/v2/workspaces/${matchedWorkspace.id}` && method === "DELETE") {
    ...
    if (path === "/api/v2/directories" && method === "GET") {
    ...
    if (path === "/api/v2/sessions" && method === "POST") {
    ...
    if (session && path === `/api/v2/sessions/${session.id}` && method === "PATCH") {
    ...
    if (session && path === `/api/v2/sessions/${session.id}` && method === "DELETE") {
    ...
    if (session && path === `/api/v2/sessions/${session.id}/mentions` && method === "GET") {
    ...
    if (session && path === `/api/v2/sessions/${session.id}/review` && method === "GET") {
      reviewRequests += 1;
      await route.fulfill({ json: { review: {
        ...
        truncated: false,
        files: [
          { path: "v2/web/src/App.tsx", indexStatus: " ", worktreeStatus: "M", patch: "diff --git a/v2/web/src/App.tsx b/v2/web/src/App.tsx\n--- a/v2/web/src/App.tsx\n+++ b/v2/web/src/App.tsx\n@@ -1,2 +1,2 @@\n-old line\n+new line", truncated: false, binary: false },
    ...
    if (session && path === `/api/v2/sessions/${session.id}/stats` && method === "GET") {
    ...
    if (session && path === `/api/v2/sessions/${session.id}/models` && method === "GET") {
      await route.fulfill({ json: { models } });
      return;
    }
    if (session && path === `/api/v2/sessions/${session.id}/configuration` && method === "POST") {
    ...
    if (session && path === `/api/v2/sessions/${session.id}/interactive` && method === "POST") {
    ...
    if (session && path === `/api/v2/sessions/${session.id}/acknowledge` && method === "POST") {
    ...
    if (session && path === `/api/v2/sessions/${session.id}/resume` && method === "POST") {
      if (body?.runtimeId !== session.runtimeId) {
        await route.fulfill({ status: 409, json: { error: "stale runtime" } });
      } else {
...
test("session drafts and delayed command failures stay scoped to their session", async ({ page }) => {
  ...
  await expect.poll(() => page.evaluate(() => localStorage.getItem("piss:v2:draft:session-1"))).toBeNull();
...
test("conversation renders coding content and remains usable at constrained heights", async ({ page }) => {
  ...
  const timestamp = new Date().toISOString();
  await page.evaluate(() => localStorage.setItem("piss:v2:draft:session-1", JSON.stringify({ text: "Render the result", delivery: "steer", updatedAt: Date.now() })));
  api.setEvents([
...
test("desktop workbench contains only operational controls", async ({ page }) => {
  ...
  await expect(page.getByRole("region", { name: "Uncommitted changes" })).toBeVisible();
  await expect(page.getByText("v2/web/src/", { exact: true })).toBeVisible();
  await expect(page.getByText("App.tsx", { exact: true })).toBeVisible();
  ...
  await expect(page.getByText(/first instruction|choose a trusted|later V2 tracer/i)).toHaveCount(0);
...
test("interactive Pi requests restore on refresh and support select, confirm, input, editor, and cancel", async ({ page }) => {
  ...
  api.setInteractiveRequests([request("input-1", "input", "Name the release", { placeholder: "v2" })] as Parameters<typeof api.setInteractiveRequests>[0]);
...
test("production PWA caches one complete build, updates atomically, and preserves offline drafts", async ({ page }) => {
  ...
  await execFileAsync("npm", ["run", "build:v2:web"], { cwd: process.cwd() });
  const directory = await mkdtemp(join(tmpdir(), "piss-v2-pwa-"));
  const buildRoot = join(directory, "public");
  await cp(resolve("dist-v2/public"), buildRoot, { recursive: true });
  ...
  const server = createServer(async (request, response) => {
    ...
    if (url.pathname.startsWith("/api/v2/")) {
      const body = url.pathname === "/api/v2/notifications" ? { supported: false }
        : url.pathname === "/api/v2/workspaces" ? { workspaces: [] }
          : url.pathname === "/api/v2/sessions" ? { sessions: [] }
            : { ok: true, version: "v2", architecture: "effect-v4" };
  ...
  try {
    ...
    const initialCache = await page.evaluate(async () => {
      ...
      const key = keys.find((candidate) => candidate.startsWith("piss-v2-shell-"));
    ...
    await page.evaluate(() => localStorage.setItem("piss:v2:draft:offline-session", JSON.stringify({ text: "offline draft survives", delivery: "steer", updatedAt: Date.now() })));
    ...
    await expect.poll(() => page.evaluate(() => JSON.parse(localStorage.getItem("piss:v2:draft:offline-session") ?? "{}").text)).toBe("offline draft edited");
    ...
    nextWorker = nextWorker.replace(/piss-v2-shell-[a-f0-9]+/, "piss-v2-shell-nextbuild");
    ...
    expect(finalCaches.keys.filter((key) => key.startsWith("piss-v2-shell-"))).toEqual(["piss-v2-shell-nextbuild"]);
