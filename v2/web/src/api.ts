...
function responseJson(response: Response): Effect.Effect<unknown, ApiError> {
  return Effect.tryPromise({
    try: () => response.json(),
    catch: (cause) => new ApiError({ message: "The V2 server returned invalid JSON", status: response.status, cause }),
  });
}
...
function request(path: string, init?: RequestInit): Effect.Effect<unknown, ApiError> {
  return Effect.tryPromise({
    ...
    catch: (cause) => new ApiError({ message: "Could not reach the V2 control plane", cause }),
  }).pipe(
      ...
      responseJson(response).pipe(
        Effect.flatMap((body) => {
          ...
          const message = typeof body === "object" && body !== null && "error" in body && typeof body.error === "string"
            ? body.error
            : `The V2 server responded with ${response.status}`;
...
export const loadNotificationCapability = request("/api/v2/notifications").pipe(
...
export function updatePushSubscription(input: unknown) {
  return request("/api/v2/notifications", {
    method: "POST",
...
export const loadWorkspaces = request("/api/v2/workspaces").pipe(
...
export function searchDirectories(query: string) {
  return request(`/api/v2/directories?query=${encodeURIComponent(query)}`).pipe(
...
export function renameWorkspace(workspaceId: string, name: string) {
  return request(`/api/v2/workspaces/${encodeURIComponent(workspaceId)}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
...
export function deleteWorkspace(workspaceId: string) {
  return request(`/api/v2/workspaces/${encodeURIComponent(workspaceId)}`, { method: "DELETE" }).pipe(Effect.asVoid);
}

export const loadSessions = request("/api/v2/sessions").pipe(
...
export function loadSession(sessionId: string) {
  return request(`/api/v2/sessions/${encodeURIComponent(sessionId)}`).pipe(
...
export function deleteOwnedSession(sessionId: string, runtimeId: string) {
  return request(`/api/v2/sessions/${encodeURIComponent(sessionId)}?runtimeId=${encodeURIComponent(runtimeId)}`, {
    method: "DELETE",
  }).pipe(Effect.asVoid);
}
...
export function searchFileMentions(sessionId: string, runtimeId: string, query: string) {
  return request(`/api/v2/sessions/${encodeURIComponent(sessionId)}/mentions?runtimeId=${encodeURIComponent(runtimeId)}&query=${encodeURIComponent(query)}`).pipe(
...
export function loadReview(sessionId: string, runtimeId: string) {
  return request(`/api/v2/sessions/${encodeURIComponent(sessionId)}/review?runtimeId=${encodeURIComponent(runtimeId)}`, { signal: AbortSignal.timeout(30_000) }).pipe(
...
export function loadAvailableModels(sessionId: string, runtimeId: string) {
  return request(`/api/v2/sessions/${encodeURIComponent(sessionId)}/models?runtimeId=${encodeURIComponent(runtimeId)}`).pipe(
...
export function loadSessionUsage(sessionId: string, runtimeId: string) {
  return request(`/api/v2/sessions/${encodeURIComponent(sessionId)}/stats?runtimeId=${encodeURIComponent(runtimeId)}`).pipe(
    Effect.flatMap(Schema.decodeUnknownEffect(OwnedSessionDetailResponse)),
    Effect.mapError((cause) => cause instanceof ApiError
      ? cause
...
export function compactSession(sessionId: string, runtimeId: string) {
  return request(`/api/v2/sessions/${encodeURIComponent(sessionId)}/configuration`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export function setSessionModel(input: {
  ...
  return request(`/api/v2/sessions/${encodeURIComponent(input.sessionId)}/configuration`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export function respondToInteractiveRequest(input: {
  ...
  return request(`/api/v2/sessions/${encodeURIComponent(input.sessionId)}/interactive`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(input),
  }).pipe(
...
export function acknowledgeOwnedSession(sessionId: string, runtimeId: string) {
  return request(`/api/v2/sessions/${encodeURIComponent(sessionId)}/acknowledge`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export function resumeOwnedSession(sessionId: string, runtimeId: string) {
  return request(`/api/v2/sessions/${encodeURIComponent(sessionId)}/resume`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ runtimeId }),
  }).pipe(
...
export function sendSessionCommand(input: {
  ...
  return request(`/api/v2/sessions/${encodeURIComponent(input.sessionId)}/commands`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
