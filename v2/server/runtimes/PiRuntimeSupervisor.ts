    ...
    yield* Effect.addFinalizer(() =>
      Effect.forEach(
        ...
        (session) => {
          ...
          failPending(session, "PISS V2 is shutting down");
      ...
      creationLock.withPermit(Effect.gen(function* () {
        ...
        const child = yield* Effect.try({
          try: () => spawn(config.piCommand, processArguments(workspace, sessionName), {
            cwd: "/proc/self/fd/3",
            env: { ...process.env, PISS_V2_OWNED_RUNTIME: "1", PISS_BRIDGE_URL: "ws://127.0.0.1:9/bridge" },
            detached: true,
