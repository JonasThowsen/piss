...
import { browserUser, validBrowserOrigin } from "../../server/browser-auth.ts";
import { validateImages } from "../../shared/protocol.ts";
import {
...
import { V2PushNotifications } from "./notifications/V2PushNotifications.ts";
...
function makeRequestHandler() {
  return Effect.gen(function* () {
    ...
    const notifications = yield* V2PushNotifications;
    ...
    return (request: IncomingMessage, response: ServerResponse): void => {
      ...
      const route = Effect.gen(function* () {
        ...
        if (pathname === "/api/v2/health" && (request.method === "GET" || request.method === "HEAD")) {
          json(response, 200, { ok: true, version: "v2", architecture: "effect-v4" });
          return;
        }
        ...
        if (pathname === "/api/v2/notifications" && request.method === "POST") {
        ...
        if (pathname === "/api/v2/workspaces" && request.method === "GET") {
          yield* requireBrowser(request, config, false);
        ...
        if (pathname === "/api/v2/workspaces" && request.method === "POST") {
        ...
        const workspaceMatch = /^\/api\/v2\/workspaces\/([^/]+)$/.exec(pathname);
        ...
        if (pathname === "/api/v2/directories" && request.method === "GET") {
          yield* requireBrowser(request, config, false);
        ...
        if (pathname === "/api/v2/sessions" && request.method === "GET") {
          yield* requireBrowser(request, config, false);
        ...
        if (pathname === "/api/v2/sessions" && request.method === "POST") {
          yield* requireBrowser(request, config, true);
        ...
        if (pathname === "/api/v2/sessions/import" && request.method === "POST") {
          yield* requireBrowser(request, config, true);
        ...
        const detailMatch = /^\/api\/v2\/sessions\/([^/]+)$/.exec(pathname);
        ...
        const mentionsMatch = /^\/api\/v2\/sessions\/([^/]+)\/mentions$/.exec(pathname);
        ...
        const reviewMatch = /^\/api\/v2\/sessions\/([^/]+)\/review$/.exec(pathname);
        ...
        const modelsMatch = /^\/api\/v2\/sessions\/([^/]+)\/models$/.exec(pathname);
        ...
        const statsMatch = /^\/api\/v2\/sessions\/([^/]+)\/stats$/.exec(pathname);
        ...
        const configurationMatch = /^\/api\/v2\/sessions\/([^/]+)\/configuration$/.exec(pathname);
        ...
        const interactiveMatch = /^\/api\/v2\/sessions\/([^/]+)\/interactive$/.exec(pathname);
        ...
        const acknowledgeMatch = /^\/api\/v2\/sessions\/([^/]+)\/acknowledge$/.exec(pathname);
        ...
        const resumeMatch = /^\/api\/v2\/sessions\/([^/]+)\/resume$/.exec(pathname);
        ...
        if (pathname === "/api/v2/notifications" || pathname === "/api/v2/workspaces" || workspaceMatch || pathname === "/api/v2/directories" || pathname === "/api/v2/sessions" || pathname === "/api/v2/sessions/import" || detailMatch || mentionsMatch || reviewMatch || modelsMatch || statsMatch || configurationMatch || interactiveMatch || acknowledgeMatch || resumeMatch || commandMatch) {
        ...
        Effect.catch((error) =>
          Effect.sync(() => {
            ...
            if (requestErrorStatus(error) >= 500) console.error("PISS V2 request failed", error);
          }),
        ),
        Effect.catchCause((cause) => requestAbort.signal.aborted
          ...
          : Effect.sync(() => {
            ...
            console.error("PISS V2 request defect", cause);
          }),
        ),
      );
      ...
      void Effect.runPromise(route, { signal: requestAbort.signal }).catch((cause) => {
        if (!requestAbort.signal.aborted) {
          ...
          console.error("PISS V2 request fiber failed", cause);
        }
      }).finally(() => {
...
export const runHttpServer = Effect.gen(function* () {
  ...
  yield* Console.log(`PISS V2 listening on http://${config.host}:${config.port}`);
