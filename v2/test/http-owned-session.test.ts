...
test("serves the authenticated owned-session tracer through HTTP", async () => {
  const directory = await mkdtemp(join(tmpdir(), "piss-v2-http-"));
  const publicDir = join(directory, "public");
  await mkdir(publicDir);
  await writeFile(join(publicDir, "index.html"), "<!doctype html><title>PISS V2</title>");
  const piCommand = await fakePi(directory);
  const port = await availablePort();
  const origin = "https://piss-v2.example.ts.net";
  const identityHeaders = {
    "Tailscale-User-Login": "owner@example.com",
    "X-Forwarded-Host": "piss-v2.example.ts.net",
  };
  const base = `http://127.0.0.1:${port}`;
  const child = spawn(process.execPath, ["--import", "tsx", "v2/server/main.ts"], {
    cwd: process.cwd(),
    env: {
      ...process.env,
      NODE_ENV: "production",
      PISS_V2_HOST: "127.0.0.1",
      PISS_V2_PORT: String(port),
      PISS_V2_ALLOWED_USERS: "owner@example.com",
      PISS_V2_PUBLIC_DIR: publicDir,
      PISS_V2_STATE_DIR: join(directory, "state"),
      PISS_V2_PI_COMMAND: piCommand,
      PISS_V2_PI_SESSION_ROOTS: JSON.stringify([directory]),
      PISS_V2_WORKSPACE_DISCOVERY_ROOTS: JSON.stringify([directory]),
      PISS_V2_WORKSPACES: JSON.stringify([{ name: "HTTP test", root: directory, trustProjectResources: false }]),
  ...
  try {
    for (let attempt = 0; attempt < 100; attempt += 1) {
      try {
        const response = await fetch(`${base}/api/v2/health`);
        if (response.ok) break;
      } catch { /* server is still starting */ }
      await new Promise((resolve) => setTimeout(resolve, 20));
    }

    const unauthenticatedNotifications = await fetch(`${base}/api/v2/notifications`);
    assert.equal(unauthenticatedNotifications.status, 401);
    const notificationCapability = await fetch(`${base}/api/v2/notifications`, { headers: identityHeaders });
    ...
    const invalidSubscription = await fetch(`${base}/api/v2/notifications`, {
      method: "POST",
    ...
    const subscribed = await fetch(`${base}/api/v2/notifications`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
      body: JSON.stringify({ action: "subscribe", subscription }),
    });
    assert.equal(subscribed.status, 200, await subscribed.text());
    assert.match(await readFile(join(directory, "state", "push-subscriptions-v2.json"), "utf8"), /push\.example\.test/);
    const unsubscribed = await fetch(`${base}/api/v2/notifications`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
      body: JSON.stringify({ action: "unsubscribe", endpoint: subscription.endpoint }),
    });
    assert.equal(unsubscribed.status, 200);
    assert.doesNotMatch(await readFile(join(directory, "state", "push-subscriptions-v2.json"), "utf8"), /push\.example\.test/);

    const workspaceResponse = await fetch(`${base}/api/v2/workspaces`, { headers: identityHeaders });
    ...
    const unauthenticatedDirectories = await fetch(`${base}/api/v2/directories?query=public`);
    assert.equal(unauthenticatedDirectories.status, 401);

    const directoryResponse = await fetch(`${base}/api/v2/directories?query=public`, { headers: identityHeaders });
    ...
    const unauthenticatedWorkspace = await fetch(`${base}/api/v2/workspaces`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Origin: origin },
      body: workspaceMutationBody,
    });
    assert.equal(unauthenticatedWorkspace.status, 401);
    const wrongOriginWorkspace = await fetch(`${base}/api/v2/workspaces`, {
      method: "POST",
    ...
    const createdWorkspaceResponse = await fetch(`${base}/api/v2/workspaces`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
      body: workspaceMutationBody,
    });
    ...
    const unauthenticatedRename = await fetch(`${base}/api/v2/workspaces/${createdWorkspace.workspace.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json", Origin: origin },
      body: JSON.stringify({ name: "Renamed through HTTP" }),
    });
    assert.equal(unauthenticatedRename.status, 401);
    const wrongOriginRename = await fetch(`${base}/api/v2/workspaces/${createdWorkspace.workspace.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json", ...identityHeaders },
      body: JSON.stringify({ name: "Renamed through HTTP" }),
    });
    assert.equal(wrongOriginRename.status, 403);
    const renamedWorkspaceResponse = await fetch(`${base}/api/v2/workspaces/${createdWorkspace.workspace.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
      body: JSON.stringify({ name: "Renamed through HTTP" }),
    });
    ...
    const removableWorkspaceResponse = await fetch(`${base}/api/v2/workspaces`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
    ...
    const removedWorkspaceResponse = await fetch(`${base}/api/v2/workspaces/${removableWorkspace.workspace.id}`, {
      method: "DELETE",
    ...
    const workspacesAfterRemoval = await fetch(`${base}/api/v2/workspaces`, { headers: identityHeaders });
    ...
    const racedWorkspaceResponse = await fetch(`${base}/api/v2/workspaces`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
    ...
    const [racedRemoval, racedCreation] = await Promise.all([
      fetch(`${base}/api/v2/workspaces/${racedWorkspace.workspace.id}`, { method: "DELETE", headers: { Origin: origin, ...identityHeaders } }),
      fetch(`${base}/api/v2/sessions`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
    ...
    if (racedCreation.status === 201) {
      ...
      await fetch(`${base}/api/v2/sessions/${racedSession.session.id}/commands`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
        body: JSON.stringify({ runtimeId: racedSession.session.runtimeId, action: "stop" }),
      });
      await fetch(`${base}/api/v2/sessions/${racedSession.session.id}?runtimeId=${racedSession.session.runtimeId}`, { method: "DELETE", headers: { Origin: origin, ...identityHeaders } });
      const cleanupWorkspace = await fetch(`${base}/api/v2/workspaces/${racedWorkspace.workspace.id}`, { method: "DELETE", headers: { Origin: origin, ...identityHeaders } });
      assert.equal(cleanupWorkspace.status, 200);
    }

    const rejected = await fetch(`${base}/api/v2/sessions`, {
      method: "POST",
      headers: { "Content-Type": "application/json", ...identityHeaders },
    ...
    const createdResponse = await fetch(`${base}/api/v2/sessions`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
    ...
    const occupiedWorkspaceRemoval = await fetch(`${base}/api/v2/workspaces/${createdWorkspace.workspace.id}`, {
      method: "DELETE",
      headers: { Origin: origin, ...identityHeaders },
    });
    ...
    for (let attempt = 0; attempt < 100; attempt += 1) {
      const response = await fetch(`${base}/api/v2/sessions`, { headers: identityHeaders });
    ...
    const acknowledgedResponse = await fetch(`${base}/api/v2/sessions/${created.session.id}/acknowledge`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
    ...
    const detailResponse = await fetch(`${base}/api/v2/sessions/${created.session.id}`, { headers: identityHeaders });
    ...
    const staleRename = await fetch(`${base}/api/v2/sessions/${created.session.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
      body: JSON.stringify({ runtimeId: "stale", name: "Must not apply" }),
    });
    assert.equal(staleRename.status, 409);
    const renameResponse = await fetch(`${base}/api/v2/sessions/${created.session.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
    ...
    const reviewPath = `${base}/api/v2/sessions/${created.session.id}/review?runtimeId=${created.session.runtimeId}`;
    const unauthenticatedReview = await fetch(reviewPath);
    assert.equal(unauthenticatedReview.status, 401);
    const staleReview = await fetch(`${base}/api/v2/sessions/${created.session.id}/review?runtimeId=stale`, { headers: identityHeaders });
    ...
    const unauthenticatedMentions = await fetch(`${base}/api/v2/sessions/${created.session.id}/mentions?runtimeId=${created.session.runtimeId}&query=mention`);
    assert.equal(unauthenticatedMentions.status, 401);
    const staleMentions = await fetch(`${base}/api/v2/sessions/${created.session.id}/mentions?runtimeId=stale&query=mention`, { headers: identityHeaders });
    assert.equal(staleMentions.status, 409);
    const mentionsResponse = await fetch(`${base}/api/v2/sessions/${created.session.id}/mentions?runtimeId=${created.session.runtimeId}&query=mention`, { headers: identityHeaders });
    ...
    const unauthenticatedModels = await fetch(`${base}/api/v2/sessions/${created.session.id}/models?runtimeId=${created.session.runtimeId}`);
    assert.equal(unauthenticatedModels.status, 401);
    const staleModels = await fetch(`${base}/api/v2/sessions/${created.session.id}/models?runtimeId=stale`, { headers: identityHeaders });
    assert.equal(staleModels.status, 409);
    const modelsResponse = await fetch(`${base}/api/v2/sessions/${created.session.id}/models?runtimeId=${created.session.runtimeId}`, { headers: identityHeaders });
    ...
    const wrongOriginConfiguration = await fetch(`${base}/api/v2/sessions/${created.session.id}/configuration`, {
      method: "POST",
      headers: { "Content-Type": "application/json", ...identityHeaders },
      body: thinkingMutation,
    });
    assert.equal(wrongOriginConfiguration.status, 403);
    const staleConfiguration = await fetch(`${base}/api/v2/sessions/${created.session.id}/configuration`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
      body: JSON.stringify({ runtimeId: "stale", action: "setThinkingLevel", level: "high" }),
    });
    assert.equal(staleConfiguration.status, 409);
    const thinkingResponse = await fetch(`${base}/api/v2/sessions/${created.session.id}/configuration`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
      body: thinkingMutation,
    });
    ...
    const modelResponse = await fetch(`${base}/api/v2/sessions/${created.session.id}/configuration`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
    ...
    const configuredDetail = await fetch(`${base}/api/v2/sessions/${created.session.id}`, { headers: identityHeaders });
    assert.doesNotMatch(await configuredDetail.text(), /super-secret|credential@example/);

    const usageResponse = await fetch(`${base}/api/v2/sessions/${created.session.id}/stats?runtimeId=${created.session.runtimeId}`, { headers: identityHeaders });
    ...
    const autoCompactionResponse = await fetch(`${base}/api/v2/sessions/${created.session.id}/configuration`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
    ...
    const compactionResponse = await fetch(`${base}/api/v2/sessions/${created.session.id}/configuration`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
    ...
    const siblingResponse = await fetch(`${base}/api/v2/sessions`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
    ...
    const unauthenticatedDelete = await fetch(`${base}/api/v2/sessions/${created.session.id}?runtimeId=${created.session.runtimeId}`, {
      method: "DELETE",
      headers: { Origin: origin },
    });
    assert.equal(unauthenticatedDelete.status, 401);
    const crossOriginDelete = await fetch(`${base}/api/v2/sessions/${created.session.id}?runtimeId=${created.session.runtimeId}`, {
      method: "DELETE",
      headers: { Origin: "https://evil.example", ...identityHeaders },
    });
    assert.equal(crossOriginDelete.status, 403);
    const staleDelete = await fetch(`${base}/api/v2/sessions/${created.session.id}?runtimeId=stale-runtime`, {
      method: "DELETE",
      headers: { Origin: origin, ...identityHeaders },
    });
    assert.equal(staleDelete.status, 409);
    const activeDelete = await fetch(`${base}/api/v2/sessions/${created.session.id}?runtimeId=${created.session.runtimeId}`, {
      method: "DELETE",
      headers: { Origin: origin, ...identityHeaders },
    });
    assert.equal(activeDelete.status, 409);

    const stale = await fetch(`${base}/api/v2/sessions/${created.session.id}/commands`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
    ...
    const stopped = await fetch(`${base}/api/v2/sessions/${created.session.id}/commands`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
      body: JSON.stringify({ runtimeId: created.session.runtimeId, action: "stop" }),
    });
    assert.equal(stopped.status, 202, await stopped.text());
    const resumedResponse = await fetch(`${base}/api/v2/sessions/${created.session.id}/resume`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
      body: JSON.stringify({ runtimeId: created.session.runtimeId }),
    });
    ...
    const staleAfterResume = await fetch(`${base}/api/v2/sessions/${created.session.id}/commands`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
      body: JSON.stringify({ runtimeId: created.session.runtimeId, action: "abort" }),
    });
    assert.equal(staleAfterResume.status, 409);
    const restopped = await fetch(`${base}/api/v2/sessions/${created.session.id}/commands`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
    ...
    const deleted = await fetch(`${base}/api/v2/sessions/${created.session.id}?runtimeId=${resumed.session.runtimeId}`, {
      method: "DELETE",
      headers: { Origin: origin, ...identityHeaders },
    });
    if (deleted.status !== 200) assert.fail(`Session deletion failed: ${await deleted.text()}`);
    assert.deepEqual(await deleted.json(), { deleted: true });
    const deletedDetail = await fetch(`${base}/api/v2/sessions/${created.session.id}`, { headers: identityHeaders });
    assert.equal(deletedDetail.status, 404);
    const sessionsAfterDelete = await fetch(`${base}/api/v2/sessions`, { headers: identityHeaders });
    ...
    const siblingStopped = await fetch(`${base}/api/v2/sessions/${sibling.session.id}/commands`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
    ...
    const emptyResponse = await fetch(`${base}/api/v2/sessions`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
    ...
    const firstPrompt = await fetch(`${base}/api/v2/sessions/${empty.session.id}/commands`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
      body: JSON.stringify({
    ...
    const configurationWhileWorking = await fetch(`${base}/api/v2/sessions/${empty.session.id}/configuration`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
    ...
    for (const [action, text] of [["steer", "Steer while working"], ["followUp", "Follow up after settling"]] as const) {
      const queued = await fetch(`${base}/api/v2/sessions/${empty.session.id}/commands`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
    ...
    for (let attempt = 0; attempt < 100; attempt += 1) {
      const response = await fetch(`${base}/api/v2/sessions/${empty.session.id}`, { headers: identityHeaders });
    ...
    const interactiveCreate = await fetch(`${base}/api/v2/sessions`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
    ...
    const interactivePrompt = await fetch(`${base}/api/v2/sessions/${interactive.session.id}/commands`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
    ...
    for (let attempt = 0; attempt < 100; attempt += 1) {
      const response = await fetch(`${base}/api/v2/sessions/${interactive.session.id}`, { headers: identityHeaders });
    ...
    const staleInteractiveResponse = await fetch(`${base}/api/v2/sessions/${interactive.session.id}/interactive`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
    ...
    const invalidInteractiveResponse = await fetch(`${base}/api/v2/sessions/${interactive.session.id}/interactive`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
    ...
    const answeredInteractive = await fetch(`${base}/api/v2/sessions/${interactive.session.id}/interactive`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
    ...
    const stopInteractive = await fetch(`${base}/api/v2/sessions/${interactive.session.id}/commands`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
    ...
    const unauthenticatedImport = await fetch(`${base}/api/v2/sessions/import`, {
      method: "POST",
    ...
    const importResponse = await fetch(`${base}/api/v2/sessions/import`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Origin: origin, ...identityHeaders },
      body: JSON.stringify({ workspaceId, name: "Imported through HTTP", sessionFile: importedSessionFile }),
    });
