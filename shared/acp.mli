(* ACP (Agent Client Protocol) v1 envelope helpers and message constructors. The
   worker drives an ACP harness over stdio JSON-RPC; the helpers here build the
   wire shapes the harness expects and decode the envelopes the harness sends
   back.

   Naming follows the Real World OCaml "Design with Modules" conventions: this
   module is named `Acp`, the primary type is `envelope`, the codec is
   `envelope_of_yojson`, and every wire message we construct is named
   `<verb>_<noun>_<shape>` (for example `new_session_request`). *)

type request_id = string
(** The JSON-RPC `id` field. ACP allows string and integer ids; the worker
    always sends strings. *)

(** One JSON-RPC envelope read from the harness. The discriminator is the
    presence of a `method` field (Notification or Request) vs. the absence of
    `method` with an `id` (Response). *)
type envelope =
  | Response of {
      id : request_id;
...
(** Decode a single JSON-RPC envelope. Returns [Error] only when the shape is so
    malformed that the envelope kind itself is unclear. *)

val id_to_string : Yojson.Safe.t -> string option
(** Best-effort decode of a JSON-RPC `id` field. The harness is allowed to send
    string or integer ids; the worker always sends strings. Returns [None] when
    the field is missing or has an unexpected type. *)

val response_result :
  expected_id:request_id -> Yojson.Safe.t -> (Yojson.Safe.t, string) result
(** Inspect a JSON value that should be a response for a specific id. [Ok]
    carries the result payload, [Error] carries a human-readable diagnostic. *)
...
(** Build a JSON-RPC response envelope with an arbitrary id (useful when echoing
    a non-string id back). *)
...
(** The first request the worker sends to the harness: protocol version,
    advertised capabilities, and the worker identity. *)
...
val mcp_servers :
  ...
  curl_command:string ->
  Yojson.Safe.t
(** Build the `mcpServers` block carried by `session/new` and `session/load`.
    Empty [command] produces an empty list so the harness does not start a
...
val prompt_request :
  ...
  Yojson.Safe.t
(** The full `session/prompt` request. [delivery] is [None] for an initial
...
(** Strip user-uploaded image data from a `session/update` notification whose
    ...
    to its durable ledger; the raw base64 payload is replaced with the empty
    string so the log never sees the user's bytes. Other envelope kinds are
