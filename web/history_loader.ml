...
let bound_events events =
  let overflow = List.length events - warm_event_capacity in
  if overflow <= 0 then events else List.drop events overflow
...
let remember_history session_id events =
  let events = normalize_events events |> bound_events in
  store_history session_id ~events
    ~complete:(Event_history.initial_history_is_complete events)

let merge_events existing additions =
  let rec loop reversed existing additions =
    match (existing, additions) with
    | [], rest | rest, [] -> List.rev_append reversed rest
    | existing_event :: existing_rest, addition :: addition_rest ->
        let comparison =
          Int64.compare
            (Event_history.sequence existing_event)
            (Event_history.sequence addition)
        in
        if comparison < 0 then
          loop (existing_event :: reversed) existing_rest additions
        else if comparison > 0 then
          loop (addition :: reversed) existing addition_rest
        else loop (addition :: reversed) existing_rest addition_rest
  in
  loop [] existing additions

let merge_history session_id events =
  let cached = Map.find !warm_histories session_id in
  let existing =
    Option.value_map cached ~default:[] ~f:(fun cached -> cached.events)
  in
  let events =
    merge_events existing (normalize_events events) |> bound_events
  in
...
let cached_history session_id =
  ...
  | Some cached ->
      ...
      Some (cached.events, cached.complete)

let mark_history_complete session_id =
  warm_histories :=
...
let connect_stream ~selection ~session_id ~after ~inject_history
  ...
  let on_event body =
    ...
    | Ok event ->
        merge_history session_id [ event ];
        let append =
          Effect.bind
    ...
    Effect.bind
      ...
      ~f:(fun result ->
        ...
        if not (current_load generation) then Effect.Ignore
          ...
          | Ok events ->
              merge_history session_id events;
              mark_history_complete session_id;
