...
let recentToolsFromTimeline:
  (array(timelineItem), int) => array(timelineItem) = [%raw
  {|(items, limit) => {
...
module WorkingView = {
  ...
  let pulseDetail: (string, option(timelineItem), string) => string =
  ...
  let pulseNote: (string, option(timelineItem)) => string =
      (pulse, activeTool) =>
    pulseNoteImpl(pulse, Js.Nullable.fromOption(activeTool));

  let metaLine: sessionSnapshot => string = snapshot => {
  ...
  let make =
    ...
    <section
      ...
      {Array.length(recentTools) > 0
         ? <section className="working-recent" ariaLabel="Recent tool calls">
                    ...
                    <li
                      ...
                      <span
                        className={
                          "artifact-kind kind-"
                          ++ (
                            itemKind(item) == "" ? "other" : itemKind(item)
                          )
                        }>
                        {React.string(
                           itemKind(item) == "" ? "tool" : itemKind(item),
                         )}
                      </span>
...
module App = {
  ...
  let make = () => {
    ...
    let (lastObservedStatus, setLastObservedStatus) =
      React.useState(() => "");
    ...
    <main className="control-room">
            ...
            <button
              ...
              <span
                className={
                  ...
                  ++ (
                    hasActiveTool ? "running" : running ? "thinking" : "idle"
                  )
                }
              />
              {icon("activity")}
                 ...
                 ? <section
                     ...
                     <dl>
                       ...
                       <div>
                         ...
                         <dd>
                           {React.string(snapshotAgentName(snapshot))}
                         </dd>
                       </div>
                       <div>
                     ...
                     <div
                       ...
                       <div className="timeline-stream">
                         {snapshotRetentionPruned(snapshot)
                            ? <p
                                className="timeline-trimmed-notice"
                                role="status">
                                {React.string(
                                   ...
                                   ++ string_of_int(
                                        snapshotFirstSequence(snapshot),
                                      )
                    ...
                    : Array.map(
                        session => {
                          ...
                          <button
                            ...
                            onClick={_ =>
                              searchScope == "archived"
                                ? restoreSession(id) : selectSession(id)
                            }>
      ...
      {creatorOpen
         ? <ModalSurface
             ariaLabel="New session" onClose={_ => setCreatorOpen(_ => false)}>
             <form
               id="new-session-form"
               onSubmit=newSession
         ...
         ? <ModalSurface
             ...
             <ModalSurface.Footer>
               ...
               <button
                 ...
                 {React.string(submitting ? "Removing…" : "Remove workspace")}
               </button>
             </ModalSurface.Footer>
           </ModalSurface>
         : React.null}
         ...
         ? <ModalSurface
             ...
             <ModalSurface.Footer>
               ...
               <button
                 ...
                 {React.string(submitting ? "Archiving…" : "Archive session")}
               </button>
             </ModalSurface.Footer>
           </ModalSurface>
         : React.null}
