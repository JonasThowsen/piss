...
export const PushNotificationsLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const keys = yield* Effect.promise(() => loadOrCreateKeys(join(config.stateDir, "vapid-keys.json")));
    webpush.setVapidDetails(process.env.PISS_VAPID_SUBJECT ?? "mailto:piss@localhost.localdomain", keys.publicKey, keys.privateKey);
    const storagePath = join(config.stateDir, "push-subscriptions.json");
        ...
        await Promise.all([...subscriptions.values()].map(async (subscription) => {
          try {
            ...
            if (statusCode === 404 || statusCode === 410) {
              ...
              console.warn("Push notification delivery failed", { statusCode });
            }
          }
        }));
