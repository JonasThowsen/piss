  ...
  Effect.gen(function* () {
    ...
    const ensureCapacity = Ref.get(state).pipe(
      Effect.flatMap((current) => current.dynamicSeeds.length >= MAX_DYNAMIC_WORKSPACES
        ? Effect.fail(new WorkspaceStorageError({ message: `PISS supports at most ${MAX_DYNAMIC_WORKSPACES} dynamic workspaces` }))
        : Effect.void),
    );
    ...
    const add: WorkspaceRepositoryShape["add"] = (input) => lock.withPermit(Effect.gen(function* () {
      ...
      if (current.dynamicSeeds.length >= MAX_DYNAMIC_WORKSPACES) {
        return yield* Effect.fail(new WorkspaceStorageError({ message: `PISS supports at most ${MAX_DYNAMIC_WORKSPACES} dynamic workspaces` }));
      }
