...
export function workflowPhasePrompt(workflow: EngineeringWorkflow, feedback?: string): string {
  ...
  const boundary = workflowFirstIncomplete(workflow);
  const dossier = `\n\nApproved structured autonomy dossier (authoritative operation and receipt identities):\n${workflow.dossier ? JSON.stringify(workflow.dossier, null, 2) : "[legacy workflow: no structured dossier]"}`;
