...
6. A Research capability failure is not an implementation defect. For `local_only` or `targeted_external`, if Research tried to require command execution, credentials, environment preflight, or live production access outside its read/search/fetch boundary, choose `resume_with_guidance`: tell it to mark that fact `unsupported`, preserve the exact check as mandatory Plan readiness, and finish Research `ready`. Do not ask the operator to configure a Research tool or manually collect readiness output. Use `retry_transient` only for a demonstrably temporary provider failure. Require operator action only when `required_external` coverage is unavailable, the disclosure policy itself must change, or contradictory evidence prevents a safe readiness gate.
