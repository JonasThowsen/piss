...
self.addEventListener("push", (event) => {
  event.waitUntil((async () => {
    ...
    await self.registration.showNotification(typeof payload.title === "string" ? payload.title : "Pi session needs attention", {
      ...
      tag: typeof payload.tag === "string" ? payload.tag : `piss-${sessionId || "attention"}`,
...
self.addEventListener("fetch", (event) => {
  ...
  event.respondWith((async () => {
    ...
    catch { return new Response("PISS is offline", { status: 503, headers: { "content-type": "text/plain; charset=utf-8" } }); }
  })());
});
