...
- compiler-enforced persistence/worker-store/registry/workspace-I/O ownership seams replace the broad core dependency, wire-owned IDs and registry lifecycles are nominal/algebraic after adapter decoding, automatic browser recovery and catalog fan-out have explicit work/concurrency budgets, and a reproducible benchmark plus `docs/ARCHITECTURE.md` protect those decisions.
